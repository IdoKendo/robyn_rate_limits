...
class RedisFixedWindowStore(LimitStore):
    ...
    def get_calls_count(self, limit_key: str, current_timestamp: int) -> int:
        ...
        self.redis.expire(limit_key, self.window_size + self.limit_ttl)  # Ensure expiration for late calls
        return self.redis.zcard(limit_key)


class RedisTokenBucketStore(LimitStore):
    def __init__(self, redis: Redis, calls_limit: int, refill_rate: int, capacity: int = None):
        self.redis = redis
    ...
    def get_calls_count(self, limit_key: str, current_timestamp: int) -> int:
        ...
        last_refill = self.redis.get(f"{limit_key}_last_refill") or current_timestamp  # Initialize last refill
        refill_amount = (current_timestamp - int(last_refill)) * self.refill_rate  # Calculate refill tokens
        available_tokens = min(available_tokens + refill_amount, self.capacity)  # Refill, but don't exceed capacity
        self.redis.set(f"{limit_key}_last_refill", current_timestamp)  # Update last refill timestamp
